...
export const FileUploader: React.FC<FileUploaderProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Import Mode Selector card matching VBA MsgBox */}
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2">
              <Layers className="w-4 h-4 text-blue-500" />
              Chế độ Import (Xử lý dữ liệu cũ trong bảng Total)
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Tương đương hộp thoại hỏi Yes/No trong macro VBA Armature Import
            </p>
          </div>
          ...
          <div className="inline-flex rounded-lg p-1 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
            ...
            >
              Làm mới hoàn toàn (Clear & Replace)
            </button>
            <button
              id="mode-append-btn"
              type="button"
        ...
        <div className="flex flex-col items-center justify-center space-y-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Hỗ trợ định dạng: <strong>.xlsx, .xls, .xlsm, .xlsb, .csv</strong> (Có thể chọn nhiều file cùng lúc)
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 pt-1 text-[11px] text-slate-500 dark:text-slate-400">
            <span className="inline-flex items-center gap-1 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-md">
              <CheckCircle2 className="w-3 h-3 text-emerald-500" /> Tự nhận diện dòng Tiêu đề (TAG + SUPPLIER + SFI/DESC)
            </span>
            <span className="inline-flex items-center gap-1 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-md">
              <CheckCircle2 className="w-3 h-3 text-emerald-500" /> Tự động dò Alias cột
            </span>
            <span className="inline-flex items-center gap-1 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-md">
              <CheckCircle2 className="w-3 h-3 text-emerald-500" /> Tự động bỏ qua dòng trống
            </span>
          </div>
        </div>
